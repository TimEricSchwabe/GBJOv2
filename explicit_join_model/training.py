...
def plot_prediction_vs_truth(model, val_dataset, device):
    """Plot predicted vs true values for visualization"""
    model.eval()
    data = next(iter(DataLoader(val_dataset, batch_size=1, shuffle=True)))
    data = data.to(device)
...
if __name__ == "__main__":
    ...
    if config['enable_training']:
        ...
        try:
            ...
            print(f"Loaded pre-trained model from {save_path}")
        except FileNotFoundError:
            print(f"No pre-trained model found at {save_path}")
    
    # Plot prediction vs truth
    plot_prediction_vs_truth(model, val_dataset, device) 