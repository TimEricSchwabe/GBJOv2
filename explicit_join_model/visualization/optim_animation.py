#!/usr/bin/env python3
"""
Script to generate optimization animations from saved animation data.

Usage:
    python optim_animation.py <run_directory> [--fps FPS] [--tree_layout] [--max_edge_weight WEIGHT]

Example:
    python optim_animation.py optimization_results/run_20231215_143022 --fps 15 --tree_layout --max_edge_weight 2.0
"""
...
# Import the animation function from the optimization evaluation module
#from optimization_evaluation_leftlinear import create_optimization_animation
...
def create_optimization_animation(animation_data, visualization_dir, query_idx, fps=10, 
    ...
    #cost_history = animation_data.get('cost_history', [])
    penalty_history = animation_data.get('penalty_history', [])
    
    # Slice data to last N steps if requested
    ...
    def update(frame):
        ...
        #ax_graph.text(0.02, 0.98, f'Active edges: {num_active_edges}\nMax weight: {max_weight_current:.3f}', 
        #             transform=ax_graph.transAxes, fontsize=10, 
...
def main():
    ...
    max_edge_weight = 2.0
    last_n_steps = None  # Set to a number like 50 to only show last 50 steps or None to show all steps
