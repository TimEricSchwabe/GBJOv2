# -------------------  project_leftdeep.py  -------------------
"""
Projection of a real (2n-1)x(2n-1) matrix onto the set of
valid left-linear join plans.
...
import numpy as np
try:
    # SciPy ≥ 1.4 provides a fast Hungarian (a.k.a. LAPJV) implementation
    from scipy.optimize import linear_sum_assignment as hungarian
except ImportError:  # tiny fallback implementation (O(n³)), good to n≈60
    ...
    def hungarian(cost: np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        if n <= m:
            ...
            raise ValueError("rows > cols – pad manually")
        u = np.zeros(n + 1)
        v = np.zeros(n + 1)
...
def project_leftdeep_greedy_beam(
    ...
    """
    Greedy (beam_width=1) or beam-search projection without Hungarian.
