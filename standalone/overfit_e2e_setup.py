...
2. assemble three runtime packs (off / gine / gps) that share keys/counts/
   ...
   from encode_pack.py (run separately).
    ...
    cd ~/Projects/GBJOv2 && uv run python standalone/overfit_e2e_setup.py
"""
...
from train_dual import assemble, load_source
...
def extract_queries(out_json, n_groups=100):
    raw = load_source("sib", "standalone/cart_plans_9_15", "standalone/sib_plans")
...
def build_pack(variant):
    ...
    repack_model(f"standalone/models/overfit-{variant}/model_rank.pt",
...
if __name__ == "__main__":
    extract_queries("standalone/overfit_queries.json")
