...
model.npz (overfit_e2e_setup.repack_model) from the saved checkpoint.
    ...
    uv run python -m v3.mrt.finetune_mrt --model <model_rank.pt> \
        --queries v3/artifacts/queries/overfit_queries.json --dummy-oracle --epochs 3
...
from v3.core.gbjo_fast import (FastGBJO, cartesian_penalty, beam_exact,
...
def main():
    ...
    ap.add_argument("--out", default="v3/artifacts/models/mrt",
    ...
    for epoch in range(args.epochs):
        ...
        mark = ""
        # select the best checkpoint on ABSOLUTE true cost (mean picked log10 C*),
        # not regret: regret is gauged against the pool-best, so it is gameable by
        # pool collapse (regret falls while true cost rises). No early stop -- the
        # loop runs all epochs; this only governs which epoch model_mrt.pt keeps.
        if ev["mean_picked_logc"] < best_logc:
            best_logc = ev["mean_picked_logc"]
    ...
    if args.pack:                            # manifest for `build_deploy_packs --config`
        sb = os.path.join(args.out, "search_best.json")
        deploy = {"model": os.path.abspath(os.path.join(args.out, "model_mrt.pt")),
                  "src_pack": os.path.abspath(os.path.expanduser(args.pack)),
                  "steps": args.steps}
        if os.path.exists(sb):
            deploy["search_best"] = os.path.abspath(sb)
        json.dump(deploy, open(os.path.join(args.out, "deploy.json"), "w"), indent=1)
    print(f"best val mean log10 C* {best_logc:.3f}  (vs pretrained "
          f"{base['mean_picked_logc']:.3f}); best -> {args.out}/model_mrt.pt")
    if log_lams:
        print("final lambdas:",
