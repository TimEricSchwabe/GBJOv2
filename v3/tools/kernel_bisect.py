...
rdflib GBJOPlanner-det plans isolates featurization as the last variable.
    ...
    cd ~/Projects/GBJOv2 && uv run python -u -m v3.tools.kernel_bisect
"""
import json
import os
import sys
...
from v3.core.gbjo_cpp import CppGBJO
from v3.mrt.finetune_mrt import (load_emb_source, build_items, deploy_params,
                          CStarOracle, CENSORED)

PACK = os.path.expanduser(
    "~/rdflib-joinordering/gbjo_pack/full-v2-reg-mrt-20k-deploy")
MODEL = "v3/artifacts/models/full-v2-reg-mrt-20k/model_mrt.pt"
...
items = build_items(json.load(
    open("v3/artifacts/queries/mrt_queries_path20k_val100.json")), emb, counts)
oracle = CStarOracle("http://127.0.0.1:7020/", 10.0, "v3/artifacts/cache/cstar_cache.json")
...
gp = json.load(open("v3/artifacts/stats/val100_plans.json"))["plans"]["new-det"]
