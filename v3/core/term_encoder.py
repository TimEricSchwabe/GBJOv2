...
class WeightedGINEConv(GINEConv):
    """GINEConv with optional per-edge message weights (Horvitz-Thompson
    reweighting for neighborhood sampling). edge_weight=None reproduces
    GINEConv exactly, so existing models/state_dicts are unaffected."""
    ...
    def message(self, x_j, edge_attr):
        ...
        ew = getattr(self, "_edge_weight", None)
        if ew is not None:
            msg = msg * ew.view(-1, 1)
        return msg
...
class TermEncoder(nn.Module):
    def __init__(self, hidden=128, out_dim=100, n_layers=4, arch="gps",
        ...
        for _ in range(n_layers):
            conv = WeightedGINEConv(gine_mlp(), edge_dim=hidden,
                                    train_eps=False)
            if arch == "gps":
    ...
    def forward(self, occ, fanout, pe, emb, center, ntype, rel_id, edge_index,
                role_idx, batch_vec, center_pos, edge_weight=None):
