...
class CostGNNDual(nn.Module):
    ...
    def forward(self, x, edge_index, edge_index_share, batch, num_graphs=None,
        ...
        Args:
            ...
            return_nodes: also return the final per-node embeddings h (pre-pool),
                for auxiliary heads (e.g. per-join intermediate cardinality).
        Returns:
            (num_graphs,) predicted log-cost  [or (cost, h) if return_nodes]
        """
        xl = torch.sign(x) * torch.log1p(torch.abs(x))
        ...
        cost = self.fc2(g).squeeze(-1)
        return (cost, h) if return_nodes else cost
