...
from v3.core.gbjo_fast import featurize_query
from v3.core.featurize import order_to_adjacency
from v3.datagen.gen_cartesian_plans import var_sets, components, count_cart, CostOracle
...
def main():
    ...
    ap.add_argument("--checkpoint-every", type=int, default=500,
                    help="save dataset.pt every N completed queries (0=off); "
                         "lets a long run be stopped without losing work")
    ap.add_argument("--out", default="v3/artifacts/plans/sib_plans")
    ...
    out_path = os.path.join(args.out, "dataset.pt")

    def checkpoint():
        torch.save({"dataset_size": len(all_data), "triples": all_triples,
                    "data": all_data}, out_path)
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as ex:
        ...
        for done, fut in enumerate(as_completed(futs)):
            ...
            if args.checkpoint_every and (done + 1) % args.checkpoint_every == 0:
                checkpoint()
                print(f"  [checkpoint] {len(all_data)} plans -> {out_path}",
                      flush=True)
